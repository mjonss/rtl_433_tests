...
| `status_report_0x02_0x82` | Water Timer `0x02` status **report** + gateway `0x82` reply (same counter) | `Bresser-WaterTimer`, `Bresser-Gateway` | A report/reply pair in one capture: the Water Timer pushes 15 bytes of its own run state (`0x02`) and the gateway answers with `0x82` = type\|0x80, echoing `msg_counter` 6 — the reply-flag convention that also governs the acks. `gateway_time` = 15269896 is a monotonic u24 the base keeps cloud-synced (pulling the base's batteries on 2026-07-20 reset it to 0; it re-synced from the cloud on boot, not free-running uptime; ~1.07/s). Its exact unit is unconfirmed, so it is emitted raw and labelled `Status response`, not `time`. |
| `config_change_0x20_0xa0` | gateway `0x20` config change **+** valve `0xa0` ack | `Bresser-Gateway`, `Bresser-WaterTimer` | The trigger half of a config update, captured 2026-07-21 by toggling a schedule in the app on a per-channel dual-SDR capture. `config_counter=76`; the valve answers with an empty-payload `0xa0` echoing counter 8, then fetches with `0x05`/`0x06`. Answers "what tells the valve to re-read its config". |
| `run_start_0x21_0xa1` | gateway `0x21` run start **+** valve `0xa1` reply | `Bresser-Gateway`, `Bresser-WaterTimer` | A manual **misting** run started from the app: the command carries `mode=2` (misting) and `duration_s=120`, matching the 2 min set in the app, and the valve answers with `trigger=0x22` (manual+misting), `duration_s=120`, `remaining_s=121` — the same `duration+1` start value as the `0x02` countdown. First captured *manual* trigger source. |
| `run_stop_0x21_0xa1` | gateway `0x21` run stop **+** valve `0xa1` reply | `Bresser-Gateway`, `Bresser-WaterTimer` | The same run stopped 35 s later: command `mode=0`, valve replies `trigger=0x20`, `remaining_s=0` with `duration_s` still 120. Together with the start fixture this pins the `0x21` mode byte (0 stop / 1 normal / 2 misting) and shows `0xa1` is the reply to `0x21`, not a beacon. |
